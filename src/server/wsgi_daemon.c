...
int wsgi_hook_daemon_handler(conn_rec *c)
{
    ...
    if (wsgi_server_config->verbose_debugging)
    {
        ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, wsgi_server,
                     ...
                     "'%s|%d'.",
                     getpid(), c->base_server->server_hostname,
                     c->base_server->port);
    }
