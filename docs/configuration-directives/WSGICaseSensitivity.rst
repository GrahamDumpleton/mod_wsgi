...
:Default: Platform-dependent (Off on Windows and macOS, On elsewhere)
:Context: server config

When mod_wsgi is used on the Windows and macOS platforms, it will assume
...
platform. A value of ``On`` indicates that the filesystem is case
sensitive::

  WSGICaseSensitivity On

Because the directive is set in the main server config, it applies to the
whole site, so every path served by Apache must reside on a filesystem
that follows the same case convention.
