    ...
    if (self->sequence != NULL) {
	     ...
	     * would violate HTTP RFC. Note that this doesn't
            ...
            if (!done) {
			     ...
			     * as normal iterable. Note that setting of
			     * content length is actually redundant if
			     * response content already generated by
			     * calls to write() function, so check for
			     * whether prior content generated. Note
			     * that this may not be fool proof as a
			     * write() call with zero length data is by
			     * WSGI specification meant to flush headers
			     * as well.
                             */
